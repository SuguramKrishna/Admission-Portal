...
class EducationalDetail < ApplicationRecord
  ...
  validates :sslc_maths_score, presence: true, length: { minimum: 1, maximum: 3 }, format: { with: /\a\d+\z/ }
  validates :sslc_english_score, presence: true, length: { minimum: 2, maximum: 100 }, format: { with: /\a\d+\z/ }
  validates :sslc_total_score, presence: true, length: { minimum: 2, maximum: 100 }, format: { with: /\a\d+\z/ }
